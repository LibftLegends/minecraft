...
/*
 * M0 shared contract: undo/redo for src/edits/, built on the block-edit op
 * format {x, y, z, block_type, tick}. Each record additionally keeps the
 * block that was overwritten so an undo can restore it; that extra field
 * is local to this history stack and is not part of the frozen wire/save
 * format (see game_block_edit_op).
 */
...
class WorldEditHistory
{
  public:
    ...
    void record(const WorldEditRecord &entry);
    bool can_undo() const;
    bool can_redo() const;
  ...
  private:
    WorldEditRecord undo_entries_[CAPACITY];
    uint32_t undo_count_;
    WorldEditRecord redo_entries_[CAPACITY];
    uint32_t redo_count_;

    static int32_t apply(World &world, const WorldEditRecord &entry,

	...
	ft_render_framebuffer &framebuffer)
{
	...
	if (src == nullptr || framebuffer.pixels == nullptr
		|| framebuffer.width <= 0 || framebuffer.height <= 0)
		return ;
	for (int32_t y = 0; y < framebuffer.height; ++y)
	{
		src_y = (y * canvas.height()) / framebuffer.height;
		for (int32_t x = 0; x < framebuffer.width; ++x)
		{
			src_x = (x * canvas.width()) / framebuffer.width;
			framebuffer.pixels[y * framebuffer.width
				+ x] = src[static_cast<size_t>(src_y * canvas.width() + src_x)];
		}
	}
}
...
MenuController::MenuController() : canvas_(), phase_(Phase::MAIN_MENU),
	main_scene_(new MainMenuScene()), settings_scene_(new SettingsScene()),
	credits_scene_(new CreditsScene()), loading_scene_(new LoadingScene()),
	in_game_scene_(new InGameSettingsScene()), wants_start_(false),
	wants_exit_(false), dismissed_in_game_(false), wants_main_menu_(false)
{
}

MenuController::MenuController(const MenuController &other) : canvas_(),
	phase_(Phase::MAIN_MENU), main_scene_(new MainMenuScene()),

...
GpuSkyRenderer::GpuSkyRenderer() : _fullscreen_vao(0), _u_sky_size(-1),
	_u_tint_color(-1)
{
}
GpuSkyRenderer::GpuSkyRenderer(const GpuSkyRenderer &other)
{
	(void)other;
}
...
{
	std::string sky_vert = GlslLoader::load((shader_dir
				+ "sky.vert.glsl").c_str());
	std::string sky_frag = GlslLoader::load((shader_dir
				+ "sky.frag.glsl").c_str());
	std::string tint_frag = GlslLoader::load((shader_dir
				+ "tint.frag.glsl").c_str());
...
{
	_sky_shader.use();
	glUniform2f(_u_sky_size, static_cast<float>(width),
		static_cast<float>(height));
	draw_fullscreen();
}

void GpuSkyRenderer::draw_tint(float r, float g, float b,

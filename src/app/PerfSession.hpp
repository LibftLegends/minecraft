...
class PerfSession
{
  ...
  private:
	...
	static bool tick_frame(const ApplicationOptions &options,
		...
		LoopState &s);
	static bool poll_frame_input(const ApplicationOptions &options,
	...
	static void tick_world_and_render(const ApplicationOptions &options,
		...
		Camera &camera, World &world, VoxelRenderer &renderer, double dt,
		LoopState &s);
	static void run_loop(const ApplicationOptions &options,
		const RenderDistanceStrategy &strategy, ApplicationWindow &window,
		Camera &camera, World &world, VoxelRenderer &renderer, LoopState &s);
	static void print_results(uint64_t frames, double elapsed,
		uint64_t perf_hash);
};

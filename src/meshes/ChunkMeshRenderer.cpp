	...
	ClipVertex transformed_vertices[3]) const
{
	...
	bool v0 = MeshProjector::transform_vertex(camera, source_vertex_0, wx,
			wz, render_cache, &transformed_vertices[0]);
	bool v1 = MeshProjector::transform_vertex(camera, source_vertex_1, wx,
			wz, render_cache, &transformed_vertices[1]);
	bool v2 = MeshProjector::transform_vertex(camera, source_vertex_2, wx,
	...
	const ClipVertex clipped_vertices[4], size_t n) const
{
	...
	rasterizer->draw_triangle(framebuffer, depth_buffer, sv,
		source_vertex.block_id, source_vertex.face, source_vertex.packed_light);
	return (true);
}
	...
	const ClipVertex clipped_vertices[4], size_t clipped_count) const
{
	if (clipped_count == 3U)
		return (project_and_draw(framebuffer, depth_buffer, render_cache,
			source_vertex, clipped_vertices, 3U));
	if (clipped_count == 4U)
	{
		ScreenVertex sv[4];
		for (size_t i = 0; i < 4U; ++i)
			if (!MeshProjector::project_vertex(render_cache,
				...
				return (false);
		rasterizer->draw_triangle(framebuffer, depth_buffer, sv,
			source_vertex.block_id, source_vertex.face,
			source_vertex.packed_light);
		sv[1] = sv[2];
		sv[2] = sv[3];
		rasterizer->draw_triangle(framebuffer, depth_buffer, sv,
			source_vertex.block_id, source_vertex.face,
...
void ChunkMeshRenderer::process_triangle(ft_render_framebuffer &framebuffer,
	...
	ClipVertex transformed_vertices[3]) const
{
	if (!transform_triangle(camera, render_cache, world_chunk, source_vertex_0,
		source_vertex_1, source_vertex_2, transformed_vertices))
		return ;
	...
	if (all_in_front)
	{
		...
		if (p0 && p1 && p2)
			rasterizer->draw_triangle(framebuffer, depth_buffer, sv,
				source_vertex_0.block_id, source_vertex_0.face,
				source_vertex_0.packed_light);
	}
	else
	{
	...
	const RenderCache &render_cache, const WorldChunk &world_chunk) const
{
	...
	{
		const chunk_mesh_vertex &source_vertex_0 =
			world_chunk.mesh.vertices[world_chunk.mesh.indices[idx]];
		const chunk_mesh_vertex &source_vertex_1 =
			world_chunk.mesh.vertices[world_chunk.mesh.indices[idx + 1U]];
		const chunk_mesh_vertex &source_vertex_2 =
			world_chunk.mesh.vertices[world_chunk.mesh.indices[idx + 2U]];
		if (!MeshCuller::triangle_faces_camera(camera, world_chunk,
			source_vertex_0))
		{
			idx += 3U;
			continue;
		...
		process_triangle(framebuffer, depth_buffer, camera, render_cache,
			world_chunk, source_vertex_0, source_vertex_1, source_vertex_2,
			transformed);
		idx += 3U;
	}
	return (true);
